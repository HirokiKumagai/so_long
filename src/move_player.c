...
/*   Updated: 2022/10/21 14:23:12 by hkumagai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_full_collection_count(t_game_data *data)
{
	...
	if (data->map->map[data->player.y_coordinate][data->player.x_coordinate] == 'C')
	{
		data->player.get_collection_count++;
		data->map->map[data->player.y_coordinate][data->player.x_coordinate] = '0';
	}
	...
	if (max_collection_count == get_collection_count)
	{
		...
		get_put_image(data, 'O', data->exit.x_coordinate, data->exit.y_coordinate);
		return (true);
	}
	return (false);
}
...
{
	(void)direction;
	get_put_image(data, c, data->player.x_coordinate, data->player.y_coordinate);
	if (c == 'E' || c == 'O')
		data->map->count_exit--;
	if (data->map->count_exit == 0 && (c == 'E' || c == 'O'))
	{
		get_put_image(data, '0', data->player.x_coordinate, data->player.y_coordinate);
		data->map->map[data->player.y_coordinate][data->player.x_coordinate] = '0';
	}
	else if (data->map->count_exit == 0 && (c != 'E' || c != 'O'))
	{
		get_put_image(data, 'E', data->player.x_coordinate, data->player.y_coordinate);
	...
	get_put_image(data, 'P', data->player.x_coordinate, data->player.y_coordinate);
